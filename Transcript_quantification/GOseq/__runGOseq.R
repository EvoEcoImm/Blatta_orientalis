...
factor_labeling = read.table("factor_labelling.txt", row.names=2, header=F)
...
# get gene lengths
gene_lengths = read.table("bo_nbp_Trinity.gene_lengths.txt", header=T, row.names=1, com='')
gene_lengths = as.matrix(gene_lengths[,1,drop=F])


# get background gene list
background = read.table("nbp_backgroup.txt", header=T, row.names=1)
